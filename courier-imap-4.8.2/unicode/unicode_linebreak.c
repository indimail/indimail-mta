...
**
** $Id: unicode_linebreak.c,v 1.3 2011/02/19 15:18:59 mrsam Exp $
*/
...
static int unicode_lbc_callback(int value, void *ptr)
{
	...
	return (*h->cb_func)(value, h->unicode_buf[h->tail++], h->cb_arg);
}
				    ...
				    void *cb_arg)
{
	...
	if ((h->unicode_buf=(unicode_char *)malloc((h->unicode_buf_size=8)
						   * sizeof(unicode_char)))
	    == NULL)
	{
		free(h);
...
int unicode_lbc_next(unicode_lbc_info_t i, unicode_char ch)
{
	...
	if (i->head >= i->unicode_buf_size)
	{
		if (i->tail > 0)
		{
			memmove(i->unicode_buf, i->unicode_buf+i->tail,
				(i->head - i->tail) * sizeof(unicode_char));
			i->head -= i->tail;
			i->tail=0;
		}
		else
		{
			size_t new_size=i->unicode_buf_size*2;

			unicode_char *p=realloc(i->unicode_buf,
						new_size*sizeof(unicode_char));

			if (!p)
				return -1;

			i->unicode_buf=p;
			i->unicode_buf_size=new_size;
		}
	}

	i->unicode_buf[i->head++]=ch;
