 ...
 *
 * Revision 2.1  2008-09-11 16:01:40+05:30  Cprogrammer
...
sha256_crypt_r(const char *key, const char *salt, char *buffer, int buflen)
{
	...
	if (strncmp(sha256_salt_prefix, salt, sizeof(sha256_salt_prefix) - 1) == 0)
		salt += sizeof(sha256_salt_prefix) - 1; /*- Skip salt prefix.  */
	...
	if ((salt - (char *) 0) % __alignof__(uint32_t) != 0)
	{
		...
		salt = copied_salt = memcpy(tmp + __alignof__(uint32_t) - (tmp - (char *) 0) % __alignof__(uint32_t), salt, salt_len);
	}
...
sha256_crypt(const char *key, const char *salt)
{
	...
	static char    *buffer;
	static int      buflen;
	int             needed = (sizeof(sha256_salt_prefix) - 1 + sizeof(sha256_rounds_prefix) + 9 + 1 + strlen(salt) + 1 + 43 + 1);
...
} tests2[] =
{
	...
	"$5$rounds=5000$toolongsaltstring", "This is just a test",
			"$5$rounds=5000$toolongsaltstrin$Un/5jzAHMgOGZ5.mWJpuVolil07guHPvOW8" "mGRcvxa5"},
	{
	"$5$rounds=1400$anotherlongsaltstring",
			"a very much longer text to encrypt.  This one even stretches over more" "than one line.",
			"$5$rounds=1400$anotherlongsalts$Rx.j8H.h8HjEDGomFU8bDkXm3XIUnzyxf12" "oP84Bnq1"},
	{
