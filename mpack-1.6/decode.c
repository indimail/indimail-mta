...
extern void     os_donewithdir(char *);
extern FILE    *os_createnewfile(char *fname);
extern char    *md5contextTo64(MD5_CTX * context);
extern void     warn(char *);
extern int      part_depth(struct part *);
extern int      handleUuencode(struct part *, char *, int);
extern void     part_ungets(char *, struct part *);
extern void     chat(char *);
extern void     part_close(struct part *);
extern int      part_fill(struct part *);
extern void     part_addboundary(struct part *, char *);
extern int      part_readboundary(struct part *);
extern void     os_warnMD5mismatch(void);
extern void     os_closetypedfile(FILE *);

/*
 * The possible content transfer encodings 
 */
enum encoding
{ enc_none, enc_qp, enc_base64 };
...
int             handlePartial(struct part *, char *, params contentParams, int);
int             ignoreMessage(struct part *);
int             handleMultipart(struct part *, char *, params contentParams, int);
int             handleText(struct part *, enum encoding contentEncoding);
int             saveToFile(struct part *, int, char *, params contentParams, enum encoding, char *, char *);
/*
...
handleMessage(struct part *inpart, char *defaultContentType, int inAppleDouble, int extractText)
{
	 ...
	 */
	headers = ParseHeaders(inpart, &subject, &contentType, &contentEncoding, &contentDisposition, &contentMD5);
	if (!headers)
		return 1;
	...
	if (!strcasecmp(contentType, "message/partial"))
	{
		if (contentEncoding != enc_none)
		{
	...
	if (!strncasecmp(contentType, "message/", 8))
	{
		/*
	...
	if (!strncasecmp(contentType, "multipart/", 10))
	{
		...
		return handleMultipart(inpart, contentType, contentParams, extractText);
	} else
	if (part_depth(inpart) == 0 && !strncasecmp(contentType, "text/", 5) && contentEncoding == enc_none &&
		!getDispositionFilename(contentDisposition) && !getParam(contentParams, "name"))
	{
		/*
	...
	if (!extractText && !inAppleDouble && !strncasecmp(contentType, "text/", 5) && !getDispositionFilename(contentDisposition) &&
	...
	} else
	{
		 ...
		 */
		return saveToFile(inpart, inAppleDouble, contentType, contentParams, contentEncoding, contentDisposition, contentMD5);
	}
}
...
char           *
ParseHeaders(struct part *inpart, char **subjectp, char **contentTypep, enum encoding *contentEncodingp, char **contentDispositionp,
			 char **contentMD5p)
{
	...
	for (next = headers; *next; next++)
	{
		if (*next == '\n')
		{
			...
			case 'C':
				...
				if (!strncasecmp(next + 2, "ontent-transfer-encoding:", 25))
				{
					*contentEncodingp = parseEncoding(next + 27);
				} else
				if (!strncasecmp(next + 2, "ontent-disposition:", 19))
				{
					val = next + 21;
					SkipWhitespace(&val);
					if (val)
				...
				if (!strncasecmp(next + 2, "ontent-md5:", 11))
				{
					val = next + 13;
					SkipWhitespace(&val);
					if (val)
...
parseEncoding(char *s)
{
	...
	if (s)
	{
		...
		case 'q':
		case 'Q':
			if (!strncasecmp(s + 1, "uoted-printable", 15) && (isspace(s[16]) || s[16] == '('))
			{
				return enc_qp;
			}
			break;
		...
		case '7':
		case '8':
			if (!strncasecmp(s + 1, "bit", 3) && (isspace(s[4]) || s[4] == '('))
			{
				return enc_none;
			}
			break;
		...
		case 'b':
		case 'B':
			if (!strncasecmp(s + 1, "ase64", 5) && (isspace(s[6]) || s[6] == '('))
			{
				return enc_base64;
			}
			if (!strncasecmp(s + 1, "inary", 5) && (isspace(s[6]) || s[6] == '('))
			{
				return enc_none;
			}
		}
...
ParseContent(char **headerp)
{
	...
	{
		while (*header && !isspace(*header) && *header != '(' && *header != ';')
		{
			*p++ = *header++;
		}
...
char           *
getParam(params cParams, char *key)
{
	static char    *value;
	static int      alloced = 0;
	int             left;
	...
	while (*cParams)
	{
		if (!strncasecmp(key, *cParams, keylen) && ((*cParams)[keylen] == '=' || isspace((*cParams)[keylen])))
			break;
		cParams++;
	}
	if (!*cParams)
...
getDispositionFilename(char *disposition)
{
	...
	for (;;)
	{
		 ...
		 * trailing whitespace.
		 */
		if (strncasecmp(disposition, "filename", 8) != 0)
			continue;
		disposition += 8;
		if (!isspace(*disposition) && *disposition != '=' && *disposition != '(')
		{
			continue;
		}
		SkipWhitespace(&disposition);
	...
	} else
	{
		 ...
		 * Just a token 
		 */
		while (*disposition && !isspace(*disposition) && *disposition != '(')
		{
			if (!--left)
			{
				alloced += VALUEGROWSIZE;
...
handlePartial(struct part *inpart, char *headers, params contentParams, int extractText)
{
	...
	if (nparts)
		sprintf(buf + strlen(buf), "of %d ", nparts);
 ...
 */
int
handleText(struct part *inpart, enum encoding contentEncoding)
{
	FILE           *descfile;
 ...
 */
int
saveToFile(struct part *inpart, int inAppleDouble, char *contentType, params contentParams, enum encoding contentEncoding,
		   char *contentDisposition, char *contentMD5)
{
	...
	if (!fname)
		fname = getParam(contentParams, "name");
	if (fname)
		fname = strsave(fname);
