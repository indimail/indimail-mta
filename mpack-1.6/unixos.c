/*-
 * $Log: unixos.c,v $
 * Revision 1.4  2008-06-13 19:25:06+05:30  Cprogrammer
 * include config.h to prevent warning for strchr
 *
 * Revision 1.3  2008-05-21 18:44:40+05:30  Cprogrammer
 * *** empty log message ***
 *
 * Revision 1.2  2004-01-06 14:55:17+05:30  Manny
 * fixed compilation warnings
 *
 * Revision 1.1  2004-01-06 12:44:17+05:30  Manny
 * Initial revision
 *
 *
...
os_idtodir(char *id)
{
	...
	*p++ = '/';
	...
	{
		if (isprint(*id) && !strchr(BADCHARS, *id))
			*p++ = *id;
		id++;
	}
	*p = '\0';
 ...
 */
FILE           *
os_newtypedfile(char *fname, char *contentType, int flags, params contentParams)
{
	char           *p;
	...
	for (p = fname; *p; p++)
	{
		...
		if (!isprint(*p) || strchr(BADCHARS, *p))
			*p = 'X';
	}
	if (!fname[0])
	{
	...
	if (!overwrite_files && (outfile = fopen(fname, "r")))
	{
		do
		{
			fclose(outfile);
