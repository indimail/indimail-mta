/*-
 * $Log: unixunpk.c,v $
 * Revision 1.2  2004-01-06 14:55:26+05:30  Manny
 * fixed compilation warnings
 *
 * Revision 1.1  2004-01-06 12:44:19+05:30  Manny
 * Initial revision
 *
 *
...
main(int argc, char **argv)
{
	...
	if (optind == argc)
	{
		...
		if (!didchat)
		{
			fprintf(stdout, "Did not find anything to unpack from standard input\n");
		}
		exit(0);
	}
	...
	while (argv[optind])
	{
		...
		} else
		{
			...
			if (!didchat)
			{
				fprintf(stdout, "Did not find anything to unpack from %s\n", argv[optind]);
			}
		}
		optind++;
	}
