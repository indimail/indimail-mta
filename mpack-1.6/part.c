/*-
 * $Log: part.c,v $
 * Revision 1.2  2004-01-06 14:55:12+05:30  Manny
 * fixed compilation warnings
 *
 * Revision 1.1  2004-01-06 12:44:15+05:30  Manny
 * Initial revision
 *
 *
...
part_addboundary(struct part *part, char *boundary)
{
	...
	if (part->boundary_num == part->boundary_alloc)
	{
		...
		part->boundary =
			(char (*)[PART_MAX_BOUNDARY_LEN + 1]) xrealloc((char *) part->boundary,
														   part->boundary_alloc * (PART_MAX_BOUNDARY_LEN + 1));
		part->boundary_length = (int *) xrealloc((char *) part->boundary_length, part->boundary_alloc * sizeof(int));
	}

	strncpy(part->boundary[part->boundary_num], boundary, PART_MAX_BOUNDARY_LEN);
	part->boundary[part->boundary_num][PART_MAX_BOUNDARY_LEN] = '\0';
	part->boundary_length[part->boundary_num] = strlen(part->boundary[part->boundary_num]);
	part->boundary_num++;
...
part_ungets(char *s, struct part *part)
{
	...
	if (part->cnt + len + 1 > part->buf_alloc)
	{
		...
		part->buf = (unsigned char *) xrealloc((char *) part->buf, part->buf_alloc);
		part->ptr = part->buf + i;
	}
...
pendingboundary(struct part *part)
{
	 ...
	 */
	if (part->cnt < 3 || (part->cnt < PART_MAX_BOUNDARY_LEN + 3 && part->ptr[1] == '-' && part->ptr[2] == '-'))
	{
	...
	for (i = 0; i < part->boundary_num; i++)
	{
		if (part->cnt - 3 >= part->boundary_length[i] &&
			!strncmp((char *) part->ptr + 3, part->boundary[i], part->boundary_length[i]))
		{
			break;
		}
	}
