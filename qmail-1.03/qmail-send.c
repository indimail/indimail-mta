...
#define CHECKSTAT if (errno != error_noent) goto fail;
	...
	if (stat(fn.s, &st) == -1)
	{
		CHECKSTAT return;	/*- someone yanking our chain */
	}
	fnmake_todo(id);
	if (stat(fn.s, &st) != -1)
...
static datetime_sec
squareroot(x)					/*- result^2 <= x < (result + 1)^2 */
