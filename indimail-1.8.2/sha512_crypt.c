 ...
 *
 * Revision 2.1  2008-09-11 23:02:48+05:30  Cprogrammer
...
sha512_crypt_r(const char *key, const char *salt, char *buffer, int buflen)
{
	...
	if (strncmp(sha512_salt_prefix, salt, sizeof(sha512_salt_prefix) - 1) == 0)
		salt += sizeof(sha512_salt_prefix) - 1; /*- Skip salt prefix.  */
	...
	if ((key - (char *) 0) % __alignof__(uint64_t) != 0)
	{
		...
		key = copied_key = memcpy(tmp + __alignof__(uint64_t) - (tmp - (char *) 0) % __alignof__(uint64_t), key, key_len);
	}
 ...
 */
	static char    *buffer;
	static int      buflen;
	int             needed = (sizeof(sha512_salt_prefix) - 1 + sizeof(sha512_rounds_prefix) + 9 + 1 + strlen(salt) + 1 + 86 + 1);
...
} tests2[] =
{
	...
	"$6$saltstring", "Hello world!",
			"$6$saltstring$svn8UoSVapNtMuq1ukKS4tPQd8iKwSMHWjl/O817G3uBnIFNjnQJu" "esI68u4OTLiBFdcbYEdFCoEOfaS35inz1"},
	{
	...
	"$6$rounds=77777$short", "we have a short salt string but not a short password",
			"$6$rounds=77777$short$WuQyW2YR.hBNpjjRhpYD/ifIw05xdfeEyQoMxIXbkvr0g" "ge1a1x3yRULJ5CCaUeOxFmtlcGZelFl5CxtgfiAc0"},
	{
