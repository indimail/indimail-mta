...
#ifndef HAVE_MEMMOVE
void           *
smemmove(To, From, count)
	void           *To;
	const void     *From;
	register size_t count;
#ifndef HAVE_BCOPY				/* silly compromise, throw */
{
	register char  *to = To;
	register const char *from = From;	/*void*old; */
/*
 * old=to;
 */ count++;
	to--;
	from--;						/* away space to be syntactically correct */
	if (to <= from) {
		goto jiasc;
		do {
...
char           *
sstrstr(phaystack, pneedle)
	const char     *const phaystack;
	const char     *const pneedle;
{
	register const uchar *haystack, *needle;
	register chartype b;
	const uchar    *rneedle;
...
#ifndef HAVE_STRTOL
long
sstrtol(start, ptr, base)
	const char     *start, **const ptr;
	int             base;
{
	long            result;
