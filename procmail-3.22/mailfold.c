...
static const char *
fifrom(fromw, lbound, ubound)
	const char     *fromw, *const lbound;
	char           *const ubound;
{
	int             i;			/* terminate & scan block */
	i = *ubound;
	*ubound = '\0';
	fromw = strstr(mx(fromw, lbound), from_expr);
	*ubound = i;
	return fromw;
}
...
static int
dirfile(chp, linkonly, type)
	char           *const chp;
	const int       linkonly, type;
{
	...
	if (type == ft_MH) {
	  ...
	  exlb:{
			...
			int             ok;
			do
				ultstr(0, ++i, chp);	/* find first empty MH folder */
	...
	if (linkonly) {
		...
		if (rlink(buf2, buf, 0))	/* hardlink the new file, it's a directory folder */
		  ...
		  didlnk:appendlastvar(buf);
								/* lastvar is "LASTFOLDER" here */
		goto ret;
	}
...
int
writefolder(boxname, linkfolder, source, len, ignwerr, dolock)
	char           *boxname, *linkfolder;
	const char     *source;
	long            len;
	const int       ignwerr, dolock;
{
	char           *chp, *chp2;
	mode_t          mode;
	int             fd, type;
	...
	case ft_FILE:
	  ...
	  dumpf:{
			switch (errno) {
			case ENOSPC:
				nlog("No space left to finish writing"), logqnl(buf);
				break;
#ifdef EDQUOT
			case EDQUOT:
				nlog("Quota exceeded while writing"), logqnl(buf);
				break;
#endif
			default:
				writeerr(buf);
			...
			for (;;) {
				...
				*chp2++ = *MCDIRSEP_;
				...
				if (!rlink(buf, buf2, 0)) {
					unlink(buf);
					break;
				} else if (errno != EEXIST && lstat(buf2, &stbuf) && errno == ENOENT && !rename(buf, buf2))
					break;
				unlink(buf);
				if (!retries--)
					goto nfail;
	...
	default:					/* case ft_DIR: */
		*chp++ = *MCDIRSEP_;
		strcpy(buf2, buf);
		chp2 = buf2 + (chp - buf);
		if (!unique(buf2, chp2, linebuf, NORMperm, verbose, 0) || 0 > (fd = dirfile(chp, 0, type)))
	  nfail:{
			nlog("Couldn't create or rename temp file");
			logqnl(buf);
			goto retf;
		}
	...
	const char     *const lstfolder;
{
	if (lgabstract > 0 || (logopened || verbose) && lgabstract) {	/* don't mail it back? */
		...
		if (mailread) {			/* is the mail completely read in? */
			...
			*thebody = i;		/* eliminate the terminator again */
			if (!nextexit &&	/* don't reenter malloc/free */
				...
				((char *) tmemmove(buf, chp2, subjlen))[subjlen] = '\0';
				detab(buf);
				elog(" ");
				elog(buf);
				elog(newline);
		...
		elog(sfolder);
		strlcpy(buf, lstfolder, MAXfoldlen);
		detab(buf);
		elog(buf);
		i = strlen(buf) + STRLEN(sfolder);
		i -= i % TABWIDTH;		/* last dump */
		do
			elog(TABCHAR);
		while ((i += TABWIDTH) < LENoffset);
		ultstr(7, lastdump, buf);
		elog(buf);
		elog(newline);
	}
}
...
void
concon(ch)
	const int       ch;			/* flip between concatenated and split fields */
{
	size_t          i;
	...
	const long      tobesent;
{
		...
		if (rhead) {			/* did we read in a new header anyway? */
		  ...
		  eofheader:
			...
			if (!berkeley &&	/* ignores Content-Length: */
				(chp = egrepin("^Content-Length:", themail.p, (long) (thebody - themail.p), 0)))
	...
	if (contlengthoffset) {
		unsigned        places;
		long            cntlen, actcntlen;
		charNUM(num, cntlen);
		chp = themail.p + contlengthoffset;
		cntlen = filled - (thebody - themail.p);
