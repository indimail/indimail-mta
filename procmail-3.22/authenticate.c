...
static void
castlower(str)
	register char  *str;		/* and I'll take the low road */
{
	for (; *str; str++)
...
#ifndef PROCMAIL
int
auth_checkpassword(pass, pw, allowemptypw)
	const auth_identity *const pass;
	const char     *const pw;
...
#else							/* PROCMAIL */
auth_identity  *auth_newid
P((void))
{
	auth_identity  *pass;		/* create a new auth_identity placeholder */
	(pass = malloc(sizeof *pass))->pw = 0;
	pass->mbox = 0;
	return pass;
}

void
auth_copyid(newpass, oldpass)
	auth_identity  *newpass;
	const auth_identity *oldpass;
{
	struct passwd  *np;
	const struct passwd *op;
	if (newpass->mbox)
	...
	np->pw_uid = (op = oldpass->pw)->pw_uid;
	np->pw_gid = op->pw_gid;
	...
	auth_identity  *pass;
{
	...
	if (p = (struct passwd *) pass->pw) {
		...
		if (p->pw_gecos)
			bbzero(p->pw_gecos, strlen(p->pw_gecos));
...
const char     *
auth_mailboxname(pass)
	auth_identity  *const pass;
{
	if (!pass->mbox)
#ifdef MAILSPOOLHOME
	{
		static const char mailfile[] = MAILSPOOLHOME;
		size_t          i;
...
#else
	{
		...
		if (!
			(pass->mbox =
			 malloc(STRLEN(mailspooldir) + MAILSPOOLHASH * 2 + strlen(pass->pw->pw_name) + 1 + STRLEN(MAILSPOOLSUFFIX))))
			return "";
		...
		; {
			...
			size_t          i;
			int             c;
			for (p = pass->mbox + STRLEN(mailspooldir), n = pass->pw->pw_name, i = MAILSPOOLHASH; i--; *p++ = '/') {
				if (*n)
					c = *n++;
				*p++ = c;
			}
