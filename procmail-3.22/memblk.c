...
#define set_fd(mb,num)	do{}while(0)
	...
	int             resizeblock(mb, len, nonfatal)
	memblk         *const mb;
	const long      len;
	const int       nonfatal;
	{
		...
		if (len > MAXinMEM && mb->fd < 0) {	/* time to switch over */
			...
			if (unique(filename, strchr(filename, '\0'), MMAP_FILE_LEN, MMAP_PERM, 0, 0)
			  ...
			  dropf:{
					...
					mb->fd = -1;
					if (verbose)
						nlog("Unable to extend or use tempfile");
				} else if (mb->len) {
	...
	char           *read2blk(mb, filledp, read_func, cleanup_func, data)
	memblk         *const mb;
	read_func_type *read_func;
	cleanup_func_type *cleanup_func;
	long           *const filledp;
	void           *data;
	{
		int             blksiz = BLKSIZ, ok;
		unsigned int    shift = EXPBLKSIZ;
		 ...
		 */
		  ...
		  jumpin:
			...
			if (EXPBLKSIZ && shift) {	/* room for growth? */
				int             newbs = blksiz;
				newbs <<= shift--;	/* capped exponential growth */
