...
lockit(name, lockp)
	...
	char          **const lockp;
{
	int             permanent = nfsTRY, triedforce = 0, locktype = doLOCK;
	struct stat     stbuf;
	time_t          t;
	zombiecollect();
	if (*lockp) {
		if (!strcmp(name, *lockp)) {	/* compare the previous lockfile to this one */
			free(name);
			return 1;			/* they're equal, save yourself some effort */
		}
	...
	for (lcking |= lck_DELAYSIG;;) {
		...
		case EEXIST:			/* check if it's time for a lock override */
			if (!lstat(name, &stbuf) && stbuf.st_size <= MAX_locksize && locktimeout && !lstat(name, &stbuf)
				&& locktimeout < t - stbuf.st_mtime)
			/*
...
int
lcllock(noext, withext)			/* lock a local lockfile */
	...
	const int       chownit;
{
	 ...
	 */
#ifndef HAVE_FDLOCK				/* watch closely :-) */
#ifdef USE_FLOCK
...
#ifdef USE_LOCKF
		if ((ret |= lockf(fd, F_TLOCK, (off_t) 0)) && (errno == EAGAIN || errno == EACCES || errno == EWOULDBLOCK))
		  ufcntl:
		{
			flck.l_type = F_UNLCK;
...
#ifdef USE_FLOCK
		if ((ret |= flock(fd, LOCK_EX | LOCK_NB)) && (errno == EAGAIN || errno == EACCES || errno == EWOULDBLOCK)) {
...
P((void))
{
	...
	; {
		...
		i |= lockf(oldfdlock, F_ULOCK, (off_t) 0);
		lseek(oldfdlock, curp, SEEK_SET);
	}
#endif
#ifdef USE_FCNTLLOCK
	flck.l_type = F_UNLCK;
