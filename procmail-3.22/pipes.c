...
P((void))
{
	lexitcode = EX_UNAVAILABLE;
	qsignal(SIGTERM, stermchild);
	qsignal(SIGINT, stermchild);
	qsignal(SIGHUP, stermchild);
	qsignal(SIGQUIT, stermchild);
	shutdesc();
}
	...
	const char     *const newname;
{
	...
	if (sh) {					/* should we start a shell? */
		...
		yell(executing, newname);
		newargv[3] = 0;
		newargv[2] = newname;
		newargv[1] = shellflags;
		*newargv = tgetenv(shell);
		...
		int             argc;
		argc = 1;
		p = newname;			/* If no shell, chop up the arguments ourselves */
		if (verbose) {
			nlog(executing);
			elog(oquote);
			goto no_1st_comma;
	...
	const long      len;
{
	...
	if (!(pidchild = sfork())) {	/* create a sending procmail */
		...
		rclose(PWRB);
		exit(EXIT_SUCCESS);		/* allow parent to proceed */
	}
	rclose(PWRB);
	rclose(PWRI);
	getstdin(PRDI);
	...
	if (Stdout) {
		char           *name;
		memblk          temp;	/* ick.  This is inefficient XXX */
		*eq = '=';
		name = Stdout;
		Stdout = 0;
		primeStdout(name);
...
long
pipin(line, source, len, asgnlastf)
	char           *const line;
	char           *source;
	long            len;
	int             asgnlastf;
