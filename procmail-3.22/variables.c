...
{"SENDMAILFLAGS", DEFflagsendmail}, {"PROCMAIL_VERSION", PROCMAIL_VERSION}
};

#define MAXvarvals	 maxindex(strenvvar)
#define MAXvarstrs	 maxindex(strenstr)

const char      lastfolder[] = "LASTFOLDER", maildir[] = "MAILDIR", scomsat[] = "COMSAT", offvalue[] = "no";
int             didchd;
long            Stdfilled;
char           *Stdout;
...
static const char slinebuf[] = "LINEBUF", pmoverflow[] = "PROCMAIL_OVERFLOW=yes", exitcode[] = "EXITCODE";
static int      setxit;
	...
	const char     *const a;
{
	...
	if (!remove) {				/* if not remove, then add it to both environments */
		...
		preenv[1] = 0;
		*(lastenv = preenv) = (char *) (split = newdynstring(&myenv, a));
		return split + eq + 1;
	}
	return empty;
}
...
void
primeStdout(varname)
	const char     *const varname;	/* changes are allowed! */
{
	if (!Stdout)
		sputenv(varname);
	Stdout = (char *) myenv;
...
void
retStdout(newmyenv, fail, unset)	/* see note on primeStdout() */
	char           *const newmyenv;
	const int       fail, unset;
{
	char           *var, *p;
...
void
retbStdout(newmyenv)
	char           *const newmyenv;	/* see note on primeStdout() */
{
	newmyenv[Stdfilled] = '\0';
	*lastenv = (myenv = (struct dynstring *) newmyenv)->ename;
	Stdout = 0;
}
	...
	const char     *const value;
{
	...
	p[len] = ' ';
	strcpy(p + len + 1, buf);
	retbStdout(p);				/* WARNING: no magic here! */
}
...
void
setdef(name, value)
	const char     *const name, *const value;
{
	char           *p;
...
void
initdefenv(pass, fallback, do_presets)
	auth_identity  *pass;
	const char     *fallback;
	int             do_presets;
{
	...
	if (pass) {
		...
		if (!p || !*p)
			p = binsh;
		setdef(shell, p);
		setdef(home, auth_homedir(pass));
		setdef(orgmail, auth_mailboxname(pass));
	} else {
		setdef(lgname, fallback);
		setdef(shell, binsh);
		setdef(home, ROOT_DIR);
		setdef(orgmail, DEAD_LETTER);
	}
...
void
setmaildir(newdir)
	const char     *const newdir;	/* destroys buf2 */
{
	char           *chp;
	didchd = 1;
	*(chp = strcpy(buf2, maildir) + STRLEN(maildir)) = '=';
	strcpy(++chp, newdir);
	sputenv(buf2);
}
	...
	int             trapisset;
{
	...
	if (setxit && (p = getenv(exitcode))) {	/* user specified exitcode? */
		...
		if (trapisset) {		/* no EXITCODE set, TRAP found, provide one */
			...
			strcpy(buf2, exitcode);
			*p = '=';
			ultstr(0, (unsigned long) retval, p + 1);
			sputenv(buf2);
		}
	}
	return forceret;
	...
	int             setenv;
{
	...
	buf = malloc(lineb + XTRAlinebuf);
	buf2 = malloc(lineb + XTRAlinebuf);
	if (setenv) {
		char           *chp;
...
asenv(chp)
	const char     *const chp;
{
	static const char logfile[] = "LOGFILE", Log[] = "LOG", sdelivered[] = "DELIVERED", includerc[] = "INCLUDERC", eumask[] =
		"UMASK", dropprivs[] = "DROPPRIVS", shift[] = "SHIFT", switchrc[] = "SWITCHRC";
	if (!strcmp(buf, slinebuf)) {
		...
		if ((i = renvint(0L, chp)) > 0) {
			if (i > crestarg)
				i = crestarg;
			crestarg -= i;
			restargv += i;		/* shift away arguments */
		}
