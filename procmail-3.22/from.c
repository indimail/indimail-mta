...
static const char From_[] = FROM, Fakefield[] = FAKE_FIELD, attemptst[] = "Attempt to fake stamp by";
	...
	long           *tobesentp;
{
	...
	return startchar;
			   ...
			   */
static char    *
findtstamp(start, end)
	const char     *start, *end;
{
	end -= 25;
	if (*start == ' ' && (++start == end || *start == ' ' && ++start == end))
		return (char *) start - 1;
	start = skpspace(start);
	start += strcspn(start, " \t\n");	/* jump over address */
...
P((void))
{
	...
	buf2[0] = buf2[1] = ' ';
	strcpy(buf2 + 2, ctime(&t));
	return strlen(buf2);
}

void
makeFrom(from, invoker)
	const char     *from, *const invoker;
{
  ...
  got_from:
	...
	if (lfr || linv) {			/* move read text beyond our From_ line */
		r = *rstart;
		tmemmove(themail.p + lfr + linv, rstart, extra);
		...
		themail.p[lfr + linv] = r;
	}
}

void
checkprivFrom_(euid, logname, override)
	uid_t           euid;
	const char     *logname;
	int             override;
{
