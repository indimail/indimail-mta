...
static void
puteps(spot, to)
	struct eps     *const spot;
	const struct eps *const to;
{
	spot->opc = OPC_EPS;
	spot->next = Ceps to;
	spot->spawn = 0;			/* epsilon transition */
}
...
static struct eps *
skiplen(ep)
	const struct eps *const ep;
{
	return epso(ep, (ep->opc & DONE_MASK) < OPC_EPS ? SZ(mchar) : skplen[(ep->opc & DONE_MASK) - OPC_EPS]);
}
...
static void
psimp(e)
	const struct eps *const e;
{
	switch (*p) {
	case R_BEG_GROUP:
		p++;					/* not so simple after all */
		if (por(e))
			errorno = 1;
		return;
	case R_BEG_CLASS:			/* a simple class */
	{
		...
		if (e) {
			r->opc = OPC_CLASS;
			r->next = Ceps e;
			Cc(r, pos1.st_) = Cc(r, pos2.st_) = 0;
			i = maxindex(rAc);
			do
				rAc[i] = j ? ~0 : 0;	/* preset the bit field */
			while (i--);
		}
	...
	const struct eps *const e;
{
	...
	for (;;) {
		...
		case R_0_OR_1:
		  ...
		  incagoon:if (endgroup(++p))
								/* at the end of this group already? */
				return;
		...
		for (pold = p, rold = r;;) {
			...
			case R_OR:
				...
				if (p == pold) {	/* empty 'or' group */
					...
					p = pold;
					pnorm(e);	/* normal 'or' group, first an */
	...
	struct eps     *down;
{
	...
	for (;;) {
		...
		case OPC_JUMP:
			down->opc = OPC_JUMP | DONE_NODE;	/* mark them as used */
		case OPC_JUMP | DONE_NODE:
			down = down->next;
			continue;
		case OPC_EPS | DONE_NODE:
			ii = 1;				/* used EPS found, return loop number */
	...
	if (!down->spawn) {			/* has it been visited (belongs to previous group?) */
		...
		down->opc = OPC_EPS | DONE_NODE;
		down->spawn = Ceps & aleps;	/* mark as used */
	...
	const unsigned  ign_case;
{
	...
	errorno = 0;
	p = (uchar *) a;
	case_ignore = ign_case;
	r = Ceps & aleps;
	cachea = 0;
	por(Ceps 0);
	st = r = malloc((i = (char *) r - (char *) &aleps) + sizeof r->opc);
			...
			if (!st->spawn) {	/* they can't be executed */
				...
				; {
					...
					for (i = r;; i = skiplen(i)) {	/* search the whole program */
						...
						case OPC_EPS | DONE_NODE | LOOPL_NODE:
							i->next = i + 1;
						case OPC_EPS | DONE_NODE | LOOPR_NODE:
							i->sp.sopc = OPC_FILL;
						case OPC_JUMP | DONE_NODE:
							i->opc = OPC_JUMP;
							continue;
...
char           *
bregexec(code, text, str, len, ign_case)
	struct eps     *code;
	...
	unsigned        ign_case;
{
	register struct eps *reg, *stack, *other, *thiss;
	unsigned        i, th1, ot1;
	struct eps     *initcode;
	const char     *eom, *pend;
	...
	ign_case = ign_case ? ~(unsigned) 0 : 0;
	eom = 0;
	stack = &sempty;
	initcode = code;
	th1 = ioffsetof(struct chclass, pos1);
	ot1 = ioffsetof(struct chclass, pos2);
	other = Ceps & tswitch;
	pend = (const char *) str + len + 1;	/* two past end */
	if (str-- == text || *str == '\n')
	...
	; {
		...
		do {
			...
			reg = initcode;
			for (;;				/* pc-stack */
				 thiss = PC(reg = thiss, th1), PC(reg, th1) = 0, start = PCp(reg, th1), reg = reg->next) {
				for (;; reg = stack->next, stack = stack->spawn)	/* pop from work-stack */
					for (;;) {
						switch (reg->opc - OPB) {
						default:
	  ...
	  checkmatch:
		if (eom) {
			static const char match[] = MATCHVAR, amatch[] = AMATCHVAR;
			char           *q;
			if (bom < (char *) text)
				bom = (const char *) text;
			if (eom > --pend)
			...
			if (getenv(match) == (const char *) text)	/* anal retentive match */
				...
				char           *p;
				primeStdout(amatch);
				p = realloc(Stdout, (Stdfilled += len) + 1);
				tmemmove(q = p + Stdfilled - (int) len, bom, len);
				retbStdout(p);
			}
			yell("Matched", q);
		}
