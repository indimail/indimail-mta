...
static char     pm_version[] = PROCMAIL_VERSION, ffileno[] = DEFfileno;
const char      offvalue[] = "no", empty[] = "";
static int      lines;
const char      dirsep[] = DIRSEP;
static const char *const keepenv[] = KEEPENV, *const prestenv[] = PRESTENV, *const trusted_ids[] = TRUSTED_IDS, *const etcrc =
	ETCRC, *const krnllocks[] = {
#ifndef NOfcntl_lock
	"\1.BR fcntl (2)",
#endif
#ifdef USElockf
	...
	int             i;
{
	...
	case '\1':
		...
		goto singesc;
	case '\2':
		i = '\\';				/* \2 doubles for nroff backslashes */
		goto singesc;
	case '\t':
		i = 't';
		goto fin;
	...
	int             supps;
{
	...
	if (!filebuf && !(filebuf = malloc(strlen(*gargv) + 1 + 4 + 1)))
		exit(EX_OSERR);
	if (lines <= 0) {
		sprintf(filebuf, "%s.%04d", *gargv, filecount++);
		freopen(filebuf, "w", stdout);
...
static void
plist(name, preamble, list, postamble, ifno, andor)
	const char     *const name, *const preamble, *const postamble, *const ifno, *const andor;
...
#endif
	plist("PRESTENV", "\1.na\1.PP\1Other cleared or preset environment variables are ", prestenv, ".\1.ad", "", " and ");
	plist("KEEPENV", ", except for the value of ", keepenv, "", "", " and ");
	plist("TRUSTED_IDS", "  If procmail is not invoked with one of the following user or group ids: ", trusted_ids, ", but still has to generate or accept a new `@FROM@' line,\1\
it will generate an additional `@FAKE_FIELD@' line to help distinguish\1\
fake mails.", "",
		  " or ");
	plist("KERNEL_LOCKING", "consistently uses the following kernel locking strategies:", krnllocks, "",
...
a security violation was found (e.g. \1.B \2-@PRESERVOPT@\1or variable\
 ...
 \2-@PRESERVOPT@\1and\1.BR \2-@FROMWHOPT@ .\1");
	...
	ps("PM_VERSION", PROCMAIL_VERSION);
	ps("BINDIR", BINDIR);
#ifdef NOpow
	pc("POW", '1');
#else
	...
	if (lines < 20)
		lines = 0;				/* make sure we have space */
	pname("AUTHORS", 1);
	putchar('c');
	p = strchr(pm_version, '\n') + 1;
	while (*p != '\n') {
