	...
	const size_t    len;
{
	...
	if (!privileged && buf && buf2) {
		buf[linebuf - 1] = buf2[linebuf - 1] = '\0';
		elog("buffer 0:");
		logqnl(buf);
		elog("buffer 1:");
		logqnl(buf2);
...
void           *
tmalloc(len)
	const size_t    len;		/* this malloc can survive a temporary */
{
	void           *p;
...
void           *
fmalloc(len)
	const size_t    len;		/* 'fragile' malloc */
{
	void           *p;
	lcking |= lck_ALLOCLIB;
	p = malloc(len);
...
frealloc(old, len)
	void           *const old;
	const size_t    len;		/* 'fragile' realloc */
{
	void           *p;
	lcking |= lck_ALLOCLIB;
	p = realloc(old, len);
...
pid_t sfork
P((void))
{								/* this fork can survive a temporary */
	pid_t           i;
	int             r;			/* "process table full" condition */
	zombiecollect();
	elog(empty);
	r = noforkretry;			/* flush log, just in case */
	while ((i = fork()) == -1) {
		lcking |= lck_FORK;
		if (!(r < 0 || r--))
			break;
...
int
rclose(fd)
	const int       fd;			/* a SysV secure close (signal immune) */
{
	int             i;
...
rread(fd, a, len)
	const int       fd, len;
	void           *const a;	/* a SysV secure read */
{
	int             i;
