	...
	const char     *start;
{
	for (;;)
		...
		case '(':
			start = skipcomment(start);	/* Thou shalt not re-curse! */
		}
}

char           *
skipwords(start)
	char           *start;		/* skips an RFC 822 address */
{
	int             delim, hitspc, machref, group;
	char           *target, *oldstart;
	group = 1;
	hitspc = machref = 0;
	target = oldstart = start;
	if (*start == '<')
		start++, machref = 1;
	for (;;) {
		switch (*start) {
		case '<':				/* machine reference */
			if (machref) {		/* cannot be nested */
				target = oldstart;
				hitspc = 0;		/* so start over */
				goto inc;
			}
			goto ret;
		case '(':
			start = (char *) skipcomment(start);	/* comment */
		case ' ':
		case '\t':
		case '\n':
		...
		case '>':
			{
				if (machref == 2) {	/* embedded ,; so you have to encapsulate it */
					*target++ = '>';
					tmemmove(oldstart + 1, oldstart, target++ - oldstart);
					*oldstart = '<';
				}
			  retz:*target = '\0';
			  ret:return start;
...
void
loadsaved(sp)
	const struct saved *const sp;	/* load some saved text */
{
	switch (*sp->rexp) {
	default:
		loadchar(' ');			/* make sure it has leading whitespace */
	case ' ':
	case '\t':;
	}
	...
	const size_t    len;
{
	...
	tmemmove(buf + buffilled, text, len);
	buffilled += len;
}

void
loadchar(c)
	const int       c;			/* append one character to buf */
{
	if (buffilled == buflen)
		buf = realloc(buf, buflen += Bsize);
	buf[buffilled++] = c;
...
void
elog(a)
	const char     *const a;	/* error output */
{
	fputs(a, stderr);
}
