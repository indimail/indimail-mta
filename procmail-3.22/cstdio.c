...
static void
refill(offset)
	const int       offset;		/* refill the buffer */
{
...
void
pushrc(name)
	const char     *const name;	/* open include rcfile */
{
	if (*name && strcmp(name, devnull)) {
		...
		if (stbuf.st_size) {	/* only if size>0 */
			...
			app_valo(inced, blasttell);
			app_vali(inced, ifdepth);	/* position, brace */
...
void
changerc(name)
	const char     *const name;	/* change rcfile */
{
	if (!*name || !strcmp(name, devnull))
  pr:{
		ifstack.filled = ifdepth;	/* lose all the braces to avoid a warning */
		rclose(rc);
		rcbufp = rcbufend + 1;	/* make it look like EOF */
		return;
	}
	if (!strcmp(name, incnamed->ename))	/* just restart this one */
		...
		if (stat(name, &stbuf) || !S_ISREG(stbuf.st_mode))
	  rerr:{
			readerr(name);		/* skip irregularities */
			return;
		}
...
void duprcs
P((void))
{								/* `duplicate' all the fds of opened rcfiles */
	size_t          i;
	struct dynstring *dp;
	dp = incnamed;
	rclose(rc);
...
int
bopen(name)
	const char     *const name;	/* my fopen */
{
	rcbufp = rcbufend = 0;
	rc = ropen(name, O_RDONLY, 0);
	if (rc >= 0) {
		char           *md;
		size_t          len;	/* if it's a relative name and an absolute $MAILDIR */
		if (!strchr(dirsep, *name) && *(md = (char *) tgetenv(maildir)) && strchr(dirsep, *md)
...
void
ungetb(x)
	const int       x;			/* only for pushing back original characters */
{
	if (x != EOF)
		rcbufp--;				/* backup */
}
...
int
testB(x)
	const int       x;			/* fgetc that only succeeds if it matches */
{
	int             i;
	if ((i = getb()) == x)
		return 1;
