...
int
rnmbogus(name, stbuf, i, dolog)
	const char     *const name;	/* move a file */
	const struct stat *const stbuf;
	const int       i, dolog;	/* out of the way */
{
	static const char renbogus[] = "Renamed bogus \"%s\" into \"%s\"", renfbogus[] =
		"Couldn't rename bogus \"%s\" into \"%s\"", bogusprefix[] = BOGUSprefix;
	char           *p;
	p = strchr(strcpy(strcpy(buf2 + i, bogusprefix) + STRLEN(bogusprefix), getenv(lgname)), '\0');
	*p++ = '.';
	ultoan((unsigned long) stbuf->st_ino, p);	/* i-node numbered */
	if (dolog) {
		nlog("Renaming bogus mailbox \"");
		elog(name);
		elog("\" info");
		logqnl(buf2);
	}
 ...
 */
static mode_t
trymkdir(dir, paranoid, i)
	const char     *const dir;
	const int       paranoid, i;
{
	struct stat     stbuf;
	int             tries = 3 - 1;	/* minus one for post-decrement */
	do {
...
static int
mkmaildir(buffer, chp, paranoid)
	char           *const buffer, *const chp;
	const int       paranoid;
{
	mode_t          mode;
	int             i = 0;
...
int
foldertype(type, forcedir, modep, paranoid)
	int             type, forcedir;
	mode_t         *const modep;
	struct stat    *const paranoid;
{
	...
	default:					/* "this cannot happen" */
		...
		ltstr(0, type, buf2);
		elog(buf2);
		elog(") passed to foldertype for folder ");
		logqnl(buf);
		Terminate();
	}
	chp = strchr(buf, '\0');
	...
	if (!S_ISDIR(mode) || (type == ft_MAILDIR && (forcedir = 1, !mkmaildir(buf, chp, paranoid != 0)))) {
		nlog("Unable to treat as directory");
		logqnl(buf);			/* we can't make it */
...
file:
  ...
  ret:
	return type;
				  ...
				  */
int
screenmailbox(chp, egid, Deliverymode)
	char           *chp;
	const gid_t     egid;
	const int       Deliverymode;
{
	char            ch;
	struct stat     stbuf;
	int             basetype, type;
 ...
 */
	...
	if (!stat(buf, &stbuf)) {
		...
		accspooldir = (wwsdir =	/* world writable spool dir? */
					   ((S_IWGRP | S_IXGRP | S_IWOTH | S_IXOTH) & stbuf.st_mode) == (S_IWGRP | S_IXGRP | S_IWOTH | S_IXOTH)
					   << 1 |	/* note it in bit 1 */
		...
		setids();
		mkdir(buf, NORMdirperm);	/* try creating the last member */
	}
	*chp = ch;
/*
 ...
 */
	 ...
	 */
		; {
			...
			if (Deliverymode && (stbuf.st_mode & S_ISUID || !S_ISDIR(stbuf.st_mode) && stbuf.st_mode & S_ISGID)) {
				nlog("Autoforwarding mailbox found\n");
				exit(EX_NOUSER);
			} else {
				if (!(stbuf.st_mode & OVERRIDE_MASK) && stbuf.st_mode & cumask & (accspooldir ? ~(mode_t) 0 : ~(S_IRGRP | S_IWGRP))) {	/* hold back */
					static const char enfperm[] = "Enforcing stricter permissions on";
					nlog(enfperm);
					logqnl(buf);
					syslog(LOG_NOTICE, slogstr, enfperm, buf);
					setids();
	...
	if (!S_ISDIR(stbuf.st_mode)) {
		...
		if (!isgrpwrite && !lstat(defdeflock, &stbuf) && stbuf.st_uid != uid && stbuf.st_uid != ROOT_uid) {
			int             i = lastdirsep(buf) - buf;
			memcpy(buf2, buf, i);
			buf2[i] = '\0';		/* try & rename bogus lockfile */
