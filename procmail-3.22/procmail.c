...
static const char *const nullp, exflags[] = RECFLAGS, drcfile[] = "Rcfile:", pmusage[] = PM_USAGE, *etcrc = ETCRC, misrecpt[] =
	"Missing recipient\n", extrns[] = "Extraneous ", ignrd[] = " ignored\n", pardir[] = chPARDIR, defspath[] =
	DEFSPATH, defmaildir[] = DEFmaildir;
char           *buf, *buf2, *loclock;
const char      shell[] = "SHELL", lockfile[] = "LOCKFILE", newline[] = "\n", binsh[] = BinSh, unexpeof[] =
	"Unexpected EOL\n", *const *gargv, *const *restargv = &nullp, *sgetcp, pmrc[] = PROCMAILRC, *rcfile, dirsep[] =
	DIRSEP, devnull[] = DevNull, empty[] = "", lgname[] = "LOGNAME", executing[] = "Executing", oquote[] = " \"", cquote[] =
	"\"\n", procmailn[] = "procmail", whilstwfor[] = " whilst waiting for ", home[] = "HOME", host[] = "HOST", *defdeflock =
	empty, *argv0 = empty, curdir[] = { chCURDIR, '\0' }, slogstr[] = "%s \"%s\"", conflicting[] = "Conflicting ", orgmail[] =
	"ORGMAIL", insufprivs[] = "Insufficient privileges\n", defpath[] = DEFPATH, exceededlb[] = "Exceeded LINEBUF\n", errwwriting[] =
	"Error while writing to", Version[] = PROCMAIL_VERSION;
int             retval = EX_CANTCREAT, retvl2 = EXIT_SUCCESS, sh, pwait, rc = -1, privileged = priv_START, lexitcode = EXIT_SUCCESS, ignwerr, crestarg, savstdout, berkeley, mailfilter, erestrict, Deliverymode, ifdepth;	/* depth of outermost */
...
static auth_identity *
savepass(spass, uid)
	auth_identity  *const spass;
	const uid_t     uid;
{
...
usage           P((void));
static int
tryopen         P((const int delay_setid, const int rctype, const int dowarning)), mainloop P((void));
	...
	const char     *const argv[];
{
	...
	; {
		...
		if (argc) {				/* sanity check, any argument at all? */
			Deliverymode = ! !strncmp(lastdirsep(argv0 = argv[0]), procmailn, STRLEN(procmailn));
			for (argc = 0; (chp2 = (char *) argv[++argc]) && *chp2 == '-';)
				for (;;) {		/* processing options */
					...
					default:
						nlog("Unrecognised options:");
						logqnl(chp2);
						elog(pmusage);
						elog("Processing continued\n");
					case '\0':;
					}
					break;
				}
	  ...
	  last_option:
		...
		; {
			auth_identity  *pass, *passinvk;
			auth_identity  *spassinvk;
			uid_t           euid = geteuid();
			uid = getuid();
			gid = getgid();
			spassinvk = auth_newid();
			passinvk = savepass(spassinvk, uid);	/* are we */
			  ...
			  dorcpt:if (enoughprivs(passinvk, euid, egid, auth_whatuid(pass), auth_whatgid(pass)))
				...
				if (mailfilter) {
					...
					; {
						 ...
						 */
							for (chp = (char *) rcfile + STRLEN(etcrcs) - 1; chp;	/* any devious embedded /../? */
								 chp = strpbrk(chp, dirsep))
								if (!strncmp(pardir, ++chp, STRLEN(pardir)) && (chp += STRLEN(pardir), strchr(dirsep, *chp)))
						 ...
						 */
							if (presenviron ||	/* -p is dangerous */
								...
								chown(buf2, ROOT_uid, stbuf.st_gid) || stbuf.st_mode & (S_IXGRP | S_IXOTH) &&	/* and accessible */
								chmod(buf2, S_IRWXU) ||	/* to no one else */
#endif
								lstat(rcfile, &stbuf) ||	/* it seems to exist */
								!enoughprivs(passinvk, euid, egid, stbuf.st_uid, stbuf.st_gid) ||	/* can we do this at all? */
		  ...
		  Setuser:{
			...
			endpwent();
			auth_freeid(spassinvk);	/* environment by default */
		}
	...
	if (rcfile && !mailfilter) {	/* "procmail rcfile..." */
		...
		for (dowarning = 1;;) {
			...
			else /* prepend default procmailrc directory */ if (buildpath(maildir, defmaildir, rcfile))
				break;
 ...
 */
static int
tryopen(delay_setid, rctype, dowarning)
	const int       delay_setid, rctype, dowarning;
{
	 ...
	 */
	{
		...
		if (((stbuf.st_uid != uid && stbuf.st_uid != ROOT_uid ||	/* check uid, */
			 ...
			 ((stbuf.st_mode & (S_IWOTH | S_IXOTH)) == (S_IWOTH | S_IXOTH) || rctype == rct_DEFAULT
			  && (stbuf.st_mode & (S_IWGRP | S_IXGRP)) == (S_IWGRP | S_IXGRP) && (NO_CHECK_stgid || stbuf.st_gid != gid)))) {
			*chp = c;
			goto suspicious_rc;
		}
		*chp = c;
...
P((void))
{
	int             lastsucc, lastcond, prevcond, i, skiprc;
	register char  *chp, *tolock;
		...
		if (testB(':')) {		/* check for a recipe */
			...
			do {
				...
				; {
					...
					nrcond = strtol(buf, &temp, 10);
					chp = temp;	/* into memory */
				}
				...
				for (tolock = 0, locknext = 0;;) {
					...
					case '\0':
						...
						break;
					case ':':
						locknext = 1;	/* yep, local lockfile specified */
						if (*chp || ++chp != Tmnate)
							tolock = tstrdup(chp), chp = strchr(chp, '\0') + 1;
					}
					concatenate(chp);
					skipped(chp);	/* display leftovers */
					break;
		  ...
		  progrm:if (testB('!')) {
				...
				else if ((chp = strchr(buf, '\0')) == buf && testB('{') && (*chp++ = '{', *chp = '\0', testB(' ') ||	/* } } */
																			testB('\t') || testB('\n'))) {
