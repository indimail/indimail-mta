...
static const char *safehost
P((void))
{								/* return a hostname safe for filenames */
	static const char *sname = 0;
	if (!sname) {
		char           *to;
		const char     *from = hostname();
		int             badchars = 0;
		...
		if (!badchars)			/* it's clean, pass it through */
			...
			while (badchars)
				...
				case '\0':
					from--;
					to--;		/* "this cannot happen" */
					break;
				case '/':
				case ':':
				case '\\':		/* we'll remap them to \ooo */
					*to++ = '\\';
					*to++ = '0' + (c >> 6);
					*to++ = '0' + ((c >> 3) & 7);
					*to++ = '0' + (c & 7);
...
int
unique(full, p, len, mode, verbos, flags)
	char           *const full;
	char           *p;
	const size_t    len;
	const mode_t    mode;
	const int       verbos, flags;
{
	static const char s2c[] = ".,+%";
	static int      serial = STRLEN(s2c);
	static time_t   t;
	char           *dot, *end, *host;
	struct stat     filebuf;
...
#define NEQ(what)	(fdbuf.what!=filebuf.what)	/* and the file? */
		if (lstat(full, &filebuf) || filebuf.st_nlink != 1 || filebuf.st_size || NEQ(st_dev) || NEQ(st_ino) || NEQ(st_uid)
			|| NEQ(st_gid) || flags & doCHOWN &&
#endif
			chown(full, uid, sgid)) {
			rclose(i);
...
{
	if (link(old, newn)) {
		register int    serrno, ret;
		struct stat     sto, stn;
		serrno = errno;
		ret = -1;
#undef NEQ						/* compare files to see if the link() */
#define NEQ(what)	(sto.what!=stn.what)	/* actually succeeded */
		if (lstat(old, &sto) || (ret = 1, lstat(newn, &stn) || NEQ(st_dev) || NEQ(st_ino) || NEQ(st_uid) || NEQ(st_gid) || S_ISLNK(sto.st_mode))) {	/* symlinks are also bad */
			SETerrno(serrno);
			if (st && ret > 0) {
	 ...
	 * SETerrno(serrno);
	 *//*
	 * we really succeeded, don't bother with errno 
	 */
	}
	return 0;
}
