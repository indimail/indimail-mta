...
void
elog(newt)
	const char     *const newt;
{
	int             lnew;
	size_t          i;
	static size_t   lold, lmax;
	static char    *old;
	...
	if (lnew &&					/* if this is not a forced flush and */
		...
		if (i > lmax) {			/* to use or expand the buffer */
			char           *p;
			size_t          newmax = lmax * 2;	/* exponential expansion by default */
		...
		lnew = 0;
		lold = i;				/* the text in newt is now in the buffer */
	...
	const unsigned long Xid;
{
	...
	if ((eff = geteuid()) != ROOT_uid && getuid() != ROOT_uid)
		return;
	syslog(LOG_CRIT, "set%cid(%lu) failed with ruid/euid = %lu/%lu", c, Xid, (unsigned long) getuid(), (unsigned long) eff);
	nlog(insufprivs);
	exit(EX_NOPERM);
}
...
void
progerr(line, xitcode, okay)
	const char     *const line;
	int             xitcode, okay;
{
	charNUM(num, thepid);
	nlog(okay ? "Non-zero exitcode (" : "Program failure (");
	ltstr(0, (long) xitcode, num);
	elog(num);
	elog(okay ? ") from" : ") of");
	logqnl(line);
}
...
int
buildpath(name, path, file)
	const char     *name, *const path, *const file;
{
	static const char toolong[] = " path too long", notabsolute[] = " is not an absolute path";
	sgetcp = path;
	if (readparse(buf, sgetc, 2, 0)) {
		syslog(LOG_CRIT, "%s%s for LINEBUF for uid \"%lu\"\n", name, toolong, (unsigned long) uid);
	  bad:nlog(name);
		elog(toolong);
		elog(newline);
		return 1;
	...
	if (!strchr(dirsep, buf[0])) {
		...
		elog(notabsolute);
		elog(newline);
		syslog(LOG_CRIT, "%s%s for uid \"%lu\"\n", name, notabsolute, (unsigned long) uid);
		return 1;
	}
	if (file) {
...
void
yell(a, b)
	const char     *const a, *const b;	/* log if VERBOSE=on */
{
	if (verbose)
		nlog(a), logqnl(b);
}
...
cstr(a, b)
	char           *const a;
	const char     *const b;	/* dynamic buffer management */
{
	if (a)
		free(a);
	return tstrdup(b);
...
P((void))
{
	...
	if (getpid() == thepid) {
		...
		if (retval != EXIT_SUCCESS) {
			...
			lstfolder = fakedelivery ? "**Lost**" : retval == EX_TEMPFAIL ? "**Requeued**" : "**Bounced**";
...
P((void))
{
	qsignal(SIGTERM, srequeue);
	qsignal(SIGINT, sbounce);
	qsignal(SIGHUP, sbounce);
	qsignal(SIGQUIT, slose);
...
char           *
egrepin(expr, source, len, casesens)
	char           *expr;
	const char     *source;
	const long      len;
	int             casesens;
...
int
enoughprivs(passinvk, euid, egid, uid, gid)
	const auth_identity *const passinvk;
	const uid_t     euid, uid;
	const gid_t     egid, gid;
{
	return euid == ROOT_uid || passinvk && auth_whatuid(passinvk) == uid || euid == uid && egid == gid;
}

const char     *
newdynstring(adrp, chp)
	struct dynstring **const adrp;
	const char     *const chp;
{
	struct dynstring *curr;
	size_t          len;
	curr = malloc(ioffsetof(struct dynstring, ename[0]) + (len = strlen(chp) + 1));
	tmemmove(curr->ename, chp, len);
	curr->enext = *adrp;
	*adrp = curr;
	return curr->ename;
}
	...
	int             size;
{
	...
	return &sp->vals[size * sp->filled++];	/* append to it */
				  ...
				  */
int
conditions(flags, prevcond, lastsucc, lastcond, skipping, nrcond)
	char            flags[];
	const int       prevcond, lastsucc, lastcond, skipping;
	int             nrcond;
{
	char           *chp, *chp2, *startchar;
	double          score;
	int             scored, i, skippedempty;
	long            tobesent;
  ...
  noconcat:
	...
	if (flags[ERROR_DO]) {
		i &= prevcond && !lastsucc;
		if (flags[ELSE_DO])
			nlog(conflicting), elog("else-if-flag"), elog(suppressed), flags[ELSE_DO] = 0;
		if (flags[ALSO_N_IF_SUCC])
			nlog(conflicting), elog("also-if-succeeded-flag"), elog(suppressed), flags[ALSO_N_IF_SUCC] = 0;
	}
	if (flags[ELSE_DO])
		i &= !prevcond;
	if (flags[ALSO_N_IF_SUCC])
	...
	for (skippedempty = 0;;) {
		...
		if (i) {				/* check out all conditions */
		  ...
		  copydone:{
				...
				default:
					chp--;		/* no special character, backup */
					{
						if (alphanum(*(chp2 = chp)) == 1) {
							...
							if (!strncmp(chp3 = skpspace(chp2), "??", 2)) {
								*chp2 = '\0';
								...
								} else if (!strcmp("HB", chp) || !strcmp("BH", chp)) {
									ltobesent = filled;
								  docon:concon(' ');
									goto partition;
								}
								ltobesent = strlen(lstartchar = (char *) tgetenv(chp));
							  partition:chp2 = skpspace(chp3 + 2);
								chp++;
								sizecheck = ltobesent;
								goto copyrest;
							}
			  ...
			  normalregexp:{
					...
					do			/* find special keyword in regexp */
						if ((chp2 = strstr(chp, regsp->regkey)) && (chp2 == buf2 || chp2[-1] != '\\')) {	/* escaped? */
							size_t          lregs, lregk;	/* no, so */
							lregk = strlen(regsp->regkey);	/* insert it */
							tmemmove(chp2 + (lregs = strlen(regsp->regsubst)), chp2 + lregk, strlen(chp2) - lregk + 1);
							tmemmove(chp2, regsp->regsubst, lregs);
							if (regsp == regs)	/* daemon regexp? */
								or_nocase = 1;	/* no case sensitivity! */
						  jinregs:regsp = regs;
											/* start over and look again */
						} else
							regsp++;	/* next keyword */
					while (regsp->regkey);
					; {
						...
						if (scoreany) {
							...
							chp = lstartchar;
							if (negate) {
								if (weight && !bregexec(re, (const uchar *) chp, (const uchar *) chp, (size_t) ltobesent, igncase))
									score += weight;
							} else {
								double          oweight = weight * weight;
								while (weight != 0 && MIN32 < score && score < MAX32 && ltobesent >= 0
									   && (chp2 =
										   bregexec(re, (const uchar *) lstartchar, (const uchar *) chp, (size_t) ltobesent,
													igncase))) {
									score += weight;
									weight *= xponent;
				...
				case '<':
				{
					...
					; {
						char           *chp3;
						pivot = strtol(buf + 1, &chp3, 10);
						chp = chp3;
					}
					skipped(skpspace(chp));
					strcpy(buf2, buf);
					if (scoreany) {
						double          f;
						if ((*buf == '<') ^ negate)
					...
					} else if (!((*buf == '<' ? sizecheck < pivot : sizecheck > pivot) ^ negate))
						i = 0;
				}
				}
				break;
