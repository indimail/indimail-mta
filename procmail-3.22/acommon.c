	...
	char           *dest;		/* convert to a number */
{
	...
	do {
		...
		*dest++ =
			i + (i < 26 ? 'A' : i < 26 + 26 ? 'a' - 26 : i < 26 + 26 + 10 ? '0' - 26 - 26 : i ==
				 26 + 26 + 10 ? '-' - 26 - 26 - 10 : '_' - 26 - 26 - 11);
	}
	while (val >>= 6);
	*dest = '\0';
	return dest;
	...
	char           *dest;
{
	...
	char           *ret;
	j = val;
	i = 0;						/* a beauty, isn't it :-) */
	do
		i++;					/* determine needed width */
	while (j /= 10);
	while (--minwidth >= i)		/* fill up any excess width */
		*dest++ = ' ';
	*(ret = dest += i) = '\0';
	do
		*--dest = '0' + val % 10;	/* display value backwards */
	while (val /= 10);
	return ret;
}
