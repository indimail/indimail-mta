...
struct field   *
findf(p, ah)
	const struct field *const p;
	register struct field **ah;
{
	size_t          i;
	int             uhead;
	char           *chp;
	register struct field *h;
...
struct field  **
addfield(pointer, text, totlen)
	struct field  **pointer;
	const char     *const text;
	const size_t    totlen;		/* add field to a linked list */
{
	register struct field *p, **pp;
	int             idlen;
	for (pp = pointer; *pp; pp = &(*pp)->fld_next);	/* skip to the end of the list */
	(*pp = p = malloc(FLD_HEADSIZ + totlen))->fld_next = 0;
	idlen = breakfield(text, totlen);
...
void
concatenate(fldp)
	struct field   *const fldp;
{
	register char  *p;
	register size_t l;			/* concatenate a continued field */
	l = fldp->Tot_len;
...
void
flushfield(pointer)
	register struct field **pointer;	/* delete and print */
{
...
P((void))
{
	addfield(&rdheader, buf, buffilled);
	buffilled = 0;
}
