...
static const struct {
	const char     *head;
	int             len, wrepl, wrrepl;
} sest[] = {
sslbar(replyto, "*********", "********"), sslbar(Fromm, "**foo***", "**bar**"), sslbar(sender, "*******", "******"),
		sslbar(res_replyto, "*", "***********"), sslbar(res_from, "*", "**********"), sslbar(res_sender, "*", "*********"),
		sslbar(path, "**", "*"), sslbar(retreceiptto, "***", "**"), sslbar(errorsto, "****", "***"), sslbar(returnpath,
																												"******",
																												"*****"),
		sslbar(From_, "*****", "****"),};

static struct saved rex[] = { bsl(subject), bsl(references), bsl(messageid), bsl(date)
};
...
const char      binsh[] = BinSh, sfolder[] = FOLDER, couldntw[] = "Couldn't write to stdout", formailn[] = FORMAILN;
...
static void logfolder
P((void))
{								/* estimate the no. of characters needed to */
	size_t          i;
	charNUM(num, Totallen);		/* represent Totallen */
	static const char tabchar[] = TABCHAR;
	if (logsummary) {
		putssn(sfolder, STRLEN(sfolder));
		putssn(logsummary, i = strlen(logsummary));
		i += STRLEN(sfolder);
		i -= i % TABWIDTH;
		do
			putssn(tabchar, STRLEN(tabchar));
		while ((i += TABWIDTH) < LENoffset);
		ultstr(7, Totallen, num);
		putssn(num, strlen(num));
		putcs('\n');
	}
}

static void
renfield(pointer, oldl, newname, newl)
	struct field  **const pointer;
	size_t          oldl;
	const size_t    newl;
	const char     *const newname;	/* rename fields */
{
	struct field   *p;
	size_t          i;
	char           *chp;
	...
	const int       sareply;
{
	...
	while (fldp) {
		struct field   *fp2;
		if (!sareply && (fp2 = findf(fldp, &iheader)) && !(areply && fldp->id_len >= fp2->Tot_len - 1)) {	/* filled replacement? */
...
P((void))
{
	...
	return i >= 0 || j > STRLEN(old_) && !strncasecmp(old_, chp, STRLEN(old_)) || j > STRLEN(x_)
		&& !strncasecmp(x_, chp, STRLEN(x_));
}

static int artheadr
P((void))
{								/* could it be the start of an article? */
	if (!rdheader && !strncmp(buf, Article_, STRLEN(Article_))) {
		addbuf();
		rdheader->id_len = STRLEN(Article_);
		return 1;
	}
	return 0;
}
				  ...
				  */
static char    *
getsender(namep, fldp, headreply)
	char           *namep;
	struct field   *fldp;
	const int       headreply;
{
	...
	if (i >= 0 && (i != maxindex(sest) || fldp == rdheader)) {	/* found anything? */
		char           *saddr;
		char           *tmp;	/* determine the weight */
		nowm = areply && headreply ? headreply == 1 ? sest[i].wrepl : sest[i].wrrepl : i;
		chp += j;
		...
		if (sest[i].head == From_) {
			...
			if (pastad = strchr(chp, ' ')) {	/* found an address? */
				...
				if (strchr(savetmp, '@'))	/* domain attached? */
					...
					strcpy(chp, savetmp);
					chp = tmp;	/* attach the user part */
				}				/* (temporary buffers might have switched) */
				free(savetmp);
				savetmp = strchr(tmp, '\0');	/* prepend '<' */
				tmemmove(tmp + 1, tmp, savetmp - tmp);
				*tmp = '<';
				savetmp[1] = '\0';
			}
		}
		...
		for (saddr = 0;; chp = skipwords(chp)) {	/* skip RFC 822 wise */
			...
			default:
				...
				continue;
			case '<':
				skipwords(saddr = chp);	/* hurray, machine useable */
			case '\0':;
			}
			break;
		}
		if (saddr) {			/* any useful mailaddress found? */
			if (*saddr) {		/* did it have any length? */
				...
				saddr = (char *) mdaemon;
				nowm = maxindex(sest) + 2;	/* override */
			  pnewname:lastm = nowm;
				saddr = strcpy(malloc(strlen(saddr) + 1), saddr);
				if (namep)
					free(namep);
				namep = saddr;
			}
	...
	const int       split;
{
	...
	if (key) {
		...
		do {
			int             j;
			char           *p;	/* start reading & comparing the next word */
	  ...
	  noluck:
		...
		fwrite(key, 1, strlen(key) + 1, idcache);
		...
		while (chp = (char *) *++argv) {
			...
			for (;;) {
				...
				case FM_ReNAME:
					...
					chp[lnl] = '\n';	/* terminate the line */
					afldp =
						addfield(lastm == FM_REN_INSERT ? &iheader : lastm == FM_DEL_INSERT ? &Iheader : lastm ==
								 FM_ADD_IFNOT ? &aheader : lastm == FM_ADD_ALWAYS ? &Aheader : lastm ==
								 FM_EXTRACT ? &xheader : lastm == FM_FIRST_UNIQ ? &uheader : lastm ==
								 FM_LAST_UNIQ ? &Uheader : lastm == FM_EXTRC_KEEP ? &Xheader : &Rheader, chp, ++lnl);
					if (lastm == FM_ReNAME) {	/* then we need a second field */
						int             copied = 0;
						for (namep = (chp = (fldp = *afldp)->fld_text) + lnl, chp += lnl = fldp->id_len; chp < namep; ++chp) {
					  ...
					  invfield:{
						...
						*afldp = fldp = realloc(fldp, FLD_HEADSIZ + (fldp->Tot_len = lnl + i));
  ...
  parsedoptions:
	...
	if (split) {
		...
		if (!vfileno) {			/* FILENO= found in the environment? */
			...
			*(vfileno = (char **) ((char *) (environ = ep) + envlen)) = 0;
			*--vfileno = ffileno;
		}						/* copy over the array */
		if ((lenfileno = strlen(chp = *vfileno + LEN_FILENO_VAR)) > STRLEN(ffileno) - LEN_FILENO_VAR - 1)	/* check the desired width */
	...
	buf = malloc(buflen = Bsize);
	Totallen = 0;
	i = maxindex(rex);			/* prime some buffers */
	do
		rex[i].rexp = malloc(1);
	while (i--);
	fdate = 0;
	addfield(&fdate, date, STRLEN(date));	/* fdate is only for searching */
	fcntlength = 0;
	addfield(&fcntlength, cntlength, STRLEN(cntlength));	/* ditto */
	fFrom_ = 0;
	addfield(&fFrom_, From_, STRLEN(From_));
	fsubject = 0;
	addfield(&fsubject, subject, STRLEN(subject));	/* likewise */
	...
	; {
		...
		if (rdheader) {
			...
			else if (babyl && !force && !strncmp(tmp, mailfrom, STRLEN(mailfrom))
		...
		if (areply) {			/* autoreply requested, we clean up the header */
			...
			loadbuf(To, STRLEN(To));
			loadchar(' ');		/* generate the To: field */
			...
			procfields(0);
				   ...
				   ((fldp = findf(fFrom_, &aheader)) && STRLEN(From_) + 1 >= fldp->Tot_len || !wasafrom_ &&	/* if there was no From_ */
					!findf(fFrom_, &iheader) &&	/* and From_ is not being */
					!findf(fFrom_, &Iheader) &&	/* supressed */
					!findf(fFrom_, &Rheader))) {
			struct field   *old;
			time_t          t;	/* insert a From_ line up front */
			t = time((time_t *) 0);
			old = rdheader;
			rdheader = 0;
		...
		for (fldp = aheader; fldp; fldp = fldp->fld_next)
			if (!findf(fldp, &rdheader))	/* only add what didn't exist */
				if (fldp->id_len + 1 >= fldp->Tot_len &&	/* field name only */
					...
					h2 = thepid;
					h3 = rhash;
					p = chp = malloc(fldp->id_len + 2 + ((sizeof h1 * 8 + 5) / 6 + 1) * 4 + strlen(name = hostname()) + 2);	/* allocate worst case length */
					memcpy(p, fldp->fld_text, fldp->id_len);
					*(p += fldp->id_len) = ' ';
					*++p = '<';
					*(p = ultoan(h3, p + 1)) = '.';
					*(p = ultoan(h4, p + 1)) = '.';
		...
		if (logsummary) {
			...
			if (fldp = findf(fsubject, &rdheader)) {
				...
				detab(chp);
				putcs(' ');
				putssn(chp, i >= MAXSUBJECTSHOW ? MAXSUBJECTSHOW : i);
				putcs('\n');
			}
		}						/* restore the saved contents of buf */
		tmemmove(buf, parkedbuf, buffilled = lenparkedbuf);
		free(parkedbuf);
	}
	flushfield(&rdheader);
	flushfield(&nheader);
	...
	if (ctlength > 0) {
		...
		if (!quiet && ctlength > 0) {
			...
			nlog(cntlength);
			elog(" field exceeds actual length by ");
			ultstr(0, (unsigned long) ctlength, num);
			elog(num);
			elog(" bytes\n");
		}
	}
	while (buffilled || !lnl || buflast != EOF) {	/* continue the quest, line by line */
			  ...
			  accuhdr:{
				  ...
				  splitit:{
						...
						if (!nowait && *argv) {	/* wait till the child has finished */
							int             excode;
							if ((excode = waitfor(child)) != EXIT_SUCCESS && retval == EXIT_SUCCESS)
								retval = excode;
						}
						if (!nrtotal)
							goto nconlyhead;
...
#ifdef sMAILBOX_SEPARATOR
				GetLine();
				buffilled = 0;	/* but only if it's defined */
#endif
		...
		if (areply && bogus)	/* escape the body */
			...
			flushfield(&rdheader);
			xheader = ox;
			Xheader = oX;		/* beware, after this buf can still be filled */
		} else
	...
	size_t          len;		/* look where the */
{
	...
	while (len) {
		...
		case ' ':
		case '\t':				/* whitespace is okay right before the colon */
