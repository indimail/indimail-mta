...
static const char lockext[] = DEFlockext, nameprefix[] = "lockfile: ", lgname[] = "LOGNAME";
...
{
	elog(nameprefix);
	elog(a);					/* decent error messages should start with this */
}
...
| -s nnn | -! | -ml | -mu | file ...\n";
  ...
  again:
	invert = (char *) progid - (char *) progid;
	qsignal(SIGHUP, failure);
	...
	for (p = argv; --argc > 0;)
		if (*(cp = (char *) *++p) == '-')
			for (cp++;;) {
				...
				case HELPOPT2:
					elog(usage);
					elog("\t-v\tdisplay the version number and exit\
...
\n\t-mu\tunlock your system mail-spool file\n");
				...
				default:
					if (sleepsec >= 0) {	/* is this still the first pass? */
					  ...
					  checkrdec:if (cp2 == cp)
					  eusg:{
							elog(usage);	/* regular usage message */
				...
				case 'm':		/* take $LOGNAME as a hint, check if valid */
				{
					...
					if (!ma) {	/* ma initialised last time? */
						if (!
							((ma = (char *) getenv(lgname)) && (pass = auth_finduser(ma, 0)) && auth_whatuid(pass) == uid
							 || (pass = auth_finduid(uid, 0)))) {
							nlog("Can't determine your mailbox, who are you?\n");
							goto nfailure;	/* panic, you're not in /etc/passwd */
						}; {
							const char     *p;
							if (!*(p = auth_mailboxname(pass)) || !(ma = malloc(strlen(p) + STRLEN(lockext) + 1)))
								goto outofmem;
			...
			while (0 > xcreat(cp, &t)) {	/* try and lock */
				...
				case EEXIST:	/* hmmm..., by force then? */
					if (force && !lstat(cp, &stbuf) && force < t - stbuf.st_mtime) {
						nlog(unlink(cp) ? "Forced unlock denied on \"" : "Forcing lock on \"");
						elog(cp);
						elog("\"\n");
				...
				case ENAMETOOLONG:
					...
					nlog("Filename too long");
					retval = EX_UNAVAILABLE;
#endif
				  lfailure:elog(", giving up on \"");
					elog(cp);
					elog("\"\n");
				  nfailure:sleepsec = -1;
					argc = p - argv;	/* mark sleepsec */
					goto again;
