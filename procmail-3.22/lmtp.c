	...
	int             flush;
{
	...
	if ((already = bufcur - buf2) + len > linebuf || flush) {
		if (already && already != rwrite(savstdout, bufcur = buf2, already) || len && len != rwrite(savstdout, buffer, len))
			exit(EX_OSERR);
	} else
...
static long slurpnumber
P((void))
{
	int             c;
	long            total = 0;	/* strtol would require buffering the number */
	...
	char           *path;
{
	...
	if (*p == '@')				/* route address.  Perhaps we should assume that */
		while (1)				/* sendmail will strip these for recipients? */
			switch (*++p) {
			case ':':
				p++;			/* we can take some shortcuts because we know */
  ...
  found:
	while (1) {					/* strip quoting */
		...
		case '"':
			...
			continue;
		case '\0':
		case '>':				/* no final host part?  That's fine with us */
		case '@':
			*q = '\0';
			return path;
		}
...
lmtp(lrout, invoker)
	struct auth_identity ***lrout;
	char           *invoker;
{
	static const char cLHLO[] = "LHLO ", cMAIL[] = "MAIL FROM:", cRCPT[] = "RCPT TO:", cDATA[] = "DATA", cBDAT[] = "BDAT", cRSET[] =
		"RSET", cVRFY[] = "VRFY ", cQUIT[] = "QUIT", cNOOP[] = "NOOP";
	const char     *msg, *msgcmd;
	int             flush = 0, c, lmtp_state = S_START;
	long            size = 0;
	...
	while (1) {
		...
		case 'Q':
			if (NOTcommand(cQUIT, 1))
				goto unknown_command;
		  quit:if (ctopfd >= 0) {
								/* we're the kid: tell the parent to quit */
...
void flushoverread
P(())
{								/* pass upwards the extra LMTP data */
	int             i;
	while (overlen) {
	...
	int             retcode;
{
	...
	if (!retcode)
		...
		message = ret2LMTP[retcode].mess;
		len = ret2LMTP[retcode].len;
	}
...
static char    *
lmtp_read_crnl(char *p, long left, void *statep)
{
...
static char    *
lmtp_read_nl(char *p, long left, void *statep)
{
	int             got, state = *(int *) statep;
	register char  *in, *q, *last;
	do {
...
lreaddyn()
{
	...
	read2blk(&themail, &filled, nliseol ? &lmtp_read_nl : &lmtp_read_crnl, (cleanup_func_type *) 0, &state);
