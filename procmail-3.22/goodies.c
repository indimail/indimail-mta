...
static const char *
evalenv(skipping)				/* expects the variable name in buf2 */
	int             skipping;
{
	int             j = buf2[0] - '0';
	return skipping ? (const char *) 0 :	/* speed this up when skipping */
		(unsigned) j > 9 ? getenv(buf2) : !j ? argv0 : j <= crestarg ? restargv[j - 1] : (const char *) 0;
}
 ...
 */
int
readparse(p, fpgetc, sarg, skipping)
	register char  *p;
	int             (*const fpgetc) ();
	const int       sarg;
	int             skipping;
{
	static int      i, skipbracelev, bracegot;
	int             got, bracelev, qbracelev;
	charNUM(num, long), *startb, *const fencepost = buf + linebuf, *const fencepost2 = buf2 + linebuf;
	static char    *skipback;
	static const char *oldstartb;
	bracelev = qbracelev = 0;
	All_args = 0;
	if (skipping)
		skipping = 2;			/* bottom bit is whether overflow occurred */
		...
		case '\\':
		  ...
		  Quoted:switch (i) {
			...
			case '`':
				goto nodelim;
			case '}':
				if (got <= NORMAL_TEXT && bracelev || got == DOUBLE_QUOTED && bracelev > qbracelev)
					goto nodelim;
			}
			if (got > NORMAL_TEXT)
			  noesc:*p++ = '\\';
								/* nothing to escape, just echo both */
			break;
		case '`':
			if (got == SINGLE_QUOTED)
				goto nodelim;
					...
					if (skipping)
						...
						*p = '\0';
						if (!(sh = ! !strpbrk(startb, shellmetas))) {
							...
							All_args = sAll_args;
							free(p);
							sgetcp = save;	/* chopped up */
						}		/* drop source buffer, read from program */
						startb = fromprog(p = startb, startb, (size_t) (buf - startb + linebuf - 3));
						sh = osh;	/* restore sh */
					}
					if (got != DOUBLE_QUOTED) {
						i = 0;
				...
				*startb = '\0';
				...
				case ':':
					...
					case '+':
						...
						goto noalt;
					default:
					  badsub:nlog("Bad substitution of");
						logqnl(buf2);
						continue;
					}
				case '+':
				...
				goto ibreak;	/* $$ =pid */
			case '$':
				ultstr(0, (unsigned long) thepid, startb = num);
				goto ieofstr;
			case '?':
				ltstr(0, (long) lexitcode, startb = num);
				goto ieofstr;
			case '#':
				ultstr(0, (unsigned long) crestarg, startb = num);
				goto ieofstr;
			case '=':
				ltstr(0, lastscore, startb = num);
			  ieofstr:i = '\0';
				goto copyit;
			case '_':
				startb = incnamed ? incnamed->ename : (char *) empty;
				goto ibreak;
			case '-':
				startb = (char *) tgetenv(lastfolder);	/* $- =$LASTFOLDER */
			  ibreak:i = '\0';
				break;
			default:
			{
				...
				if (alphanum(i)) {	/* $name */
				  ...
				  finsb:*startb = '\0';
					...
					if (quoted) {
						*p++ = '(';
						CHECKINC();	/* protect leading character */
						*p++ = ')';
...
char           *
simplesplit(to, from, fencepost, gotp)
	char           *to;
	const char     *from, *fencepost;
	int            *gotp;
{
