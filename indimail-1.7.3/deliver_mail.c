...
#ifdef HAVE_SSL
static int      is_duplicate(char *);
static int      duplicateMD5(char *, char *);
#endif
...
is_duplicate(char *maildir)
{
	...
	case 0:
		if (lseek(0, 0l, SEEK_SET) < 0)
		{
			fprintf(stderr, "is_duplicate: lseek: %s\n", strerror(errno));
			_exit(111);
		}
		close(pim[0]);
		if (dup2(pim[1], 1) == -1)
		...
		{
			fprintf(stderr, "is_duplicate: MakeArgs: %s\n", strerror(errno));
			_exit(111);
		}
		if (ptr)
