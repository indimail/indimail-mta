	...
	int             t2;
{
	if (!t1)
		return 0;
	if (t1 == TOKEN822_COLON)
		return 1;
	if (t1 == TOKEN822_COMMA)
		return 1;
	if (t2 == TOKEN822_LEFT)
		return 1;
	switch (t1)
...
#define NSUW \
 ...
 if (linee && (!linelen || (s - lineb <= linelen))) \
  { while (linee < s) { linee[0] = linee[2]; ++linee; } linee -= 2; } \
		...
		case TOKEN822_LITERAL:
		case TOKEN822_COMMENT:
			if (t->type == TOKEN822_QUOTE)
				*s++ = '"';
			if (t->type == TOKEN822_LITERAL)
				*s++ = '[';
			if (t->type == TOKEN822_COMMENT)
				*s++ = '(';
			for (j = 0; j < t->slen; ++j)
				switch (ch = t->s[j])
				{
	...
	int             (*callback) ();
{
	...
	if (!token822_readyplus(taout, 1))
		return -1;
	if (!token822_readyplus(taaddr, 1))
		return -1;
	ingroup = 0;
	wordok = 1;
	beginning = ta->t + 2;
...
#define OUTLEFT if (!token822_append(taout,t--)) return -1;
	...
	while (t >= beginning)
	{
		...
		case TOKEN822_RIGHT:
			 ...
			 */
			if (!gotaddr(taout, taaddr, callback))
				return -1;
			if (t < beginning)
				return 0;
			OUTLEFT
			while ((t >= beginning) && ((t->type == TOKEN822_COMMENT) || (t->type == TOKEN822_ATOM) || 
