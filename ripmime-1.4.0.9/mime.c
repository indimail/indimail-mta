...
{
	if (( method >= _MIME_RENAME_METHOD_INFIX ) && ( method <= _MIME_RENAME_METHOD_POSTFIX ))
	{
...
size_t MIME_read_raw( char *src_mpname, char *dest_mpname, size_t rw_buffer_size )
{
	...
	rw_buffer = malloc( rw_buffer_size *sizeof(char) );
	if ( !rw_buffer )
	{
	...
	do {
		...
		if (readcount > 0) {
			...
			{
				LOGGER_log("%s:%d:MIME_read:ERROR: Attempted to write %d bytes, but only managed %d to file '%s'",FL, readcount, writecount, dest_mpname );
			}
