/*
 * $Log: vdeluser.c,v $
 * Revision 2.11  2016-06-09 15:32:32+05:30  Cprogrammer
...
#ifndef	lint
static char     sccsid[] = "$Id: vdeluser.c,v 2.11 2016-06-09 15:32:32+05:30 Cprogrammer Stab mbhangui $";
#endif
	...
	char           *argv[];
{
	...
	if (!(ptr = getenv("POST_HANDLE")))
	{
		if (!(base_argv0 = strrchr(argv[0], '/')))
			base_argv0 = argv[0];
		return(post_handle("%s/%s %s@%s", LIBEXECDIR, base_argv0, User, Domain));
	} else
