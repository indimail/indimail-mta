/*
 * $Id: qmail-cat.c,v 1.11 2024-02-21 20:41:45+05:30 Cprogrammer Exp mbhangui $
 */
#include <unistd.h>
#include <fcntl.h>
#include <sys/types.h>
...
main(int argc, char **argv)
{
	...
	if (!(argc - optind))
		...
		for (i = optind; i < argc; i++) {
			if (!str_diff(argv[i], "-")) {
				if (dup2(0, 3) == -1)
					my_error("qmail-cat: error duplicating descriptor 0 to 3", 0, 1);
				break;
			}
		}
		for (i = optind; i < argc; i++) {
			if (!str_diff(argv[i], "-")) {
				if (dup2(3, 0) == -1)
					my_error("qmail-cat: error duplicating descriptor 3 to 0", 0, 1);
			} else {
				if (stat(argv[i], &st) == -1)
				...
				if (dup2(fd, 0) == -1)
					my_error("qmail-cat: error duplicating descriptor to 0", 0, 1);
				if (fd && close(fd))
...
{
	static char    *x = "$Id: qmail-cat.c,v 1.11 2024-02-21 20:41:45+05:30 Cprogrammer Exp mbhangui $";
...
/*
 * $Log: qmail-cat.c,v $
 * Revision 1.11  2024-02-21 20:41:45+05:30  Cprogrammer
 * added -e, -t, -v options similar to gnu cat
 *
