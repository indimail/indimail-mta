...
sigint()
{
	if (loglock_fd == -1) {
		...
		if (loglock_fd != -1) {
			close(loglock_fd);
			loglock_fd = -1;
			log5("info: ", argv0, ": ", queuedesc, ": loglock disabled\n");
		}
	}
}
#endif
	...
	for (c = 0; c < CHANNELS; ++c) {
		while (prioq_get(&pqchan[c], &pe)) {
			...
			if (utimes(fn1.s, ut) == -1)
				log7("warning: ", argv0, ": ", queuedesc, "unable to utime ",
				 ...
				 char *recipient)
{
	...
	case 0:
		...
		args[7] = 0;
		execv(*args, args);
		log9("alert: ", argv0, ": ", queuedesc, ": Unable to run: ", prog, ": ", error_str(errno), "\n");
		_exit(111);
	}
	wait_pid(&wstat, child);
	if (wait_crashed(wstat)) {
		log9("alert: ", argv0, ": ", queuedesc, ": ", prog, " crashed: ", error_str(errno), "\n");
		return (111);
	}
			...
			if (srs_domain.len) {
				...
				if (j < sender.len) {
					if (srs_domain.len == sender.len - j - 1 && stralloc_starts(&srs_domain, sender.s + j + 1)) {
						...
						{
						case -3:
							log7("alert: ", argv0, ": ", queuedesc, ": srs: ", srs_error.s, "\n");
							qmail_fail(&qqt);
							break;
						case -2:
							nomem();
...
");
		...
		case 1:/*- discard bounce */
			...
			log7("info: ", argv0, ": delete bounce: discarding ", fn2.s, " ", queuedesc, "\n");
			return 1;
		default:
			qmail_fail(&qqt);
			break;
...
#ifndef EXTERNAL_TODO
	if (control_readint(&todo_interval, "todointerval") == -1) {
		log7("alert: ", argv0, ": ", queuedesc, ": qmail-todo: unable to reread ", controldir, "/todointerval\n");
		return;
	}
#endif
	if (control_readint((int *) &concurrency[0], "concurrencylocal") == -1) {
...
run_plugin()
{
	...
	if (plugindir[i = str_chr(plugindir, '/')]) {
		log5("alert: ", argv0, ": ", queuedesc, ": plugindir cannot have an absolute path\n");
		_exit(111);
	}
	...
	for (ptr = plugin_ptr = splugin.s;; ptr++) {
		...
		if (!(handle = dlopen(plugin.s, RTLD_LAZY | RTLD_GLOBAL))) {
			log9("alert: ", argv0, ": ", queuedesc, ": dlopen failed for ", plugin.s, ": ", dlerror(), "\n");
			_exit(111);
		}
		...
		if (dlclose(handle)) {
			log9("alert: ", argv0, ": ", queuedesc, ": dlclose for ", plugin.s, "failed: ", error, "\n");
			_exit(111);
		}
		if (ptr == end)
			break;
...
{
	static char    *x = "$Id: qmail-send.c,v 1.95 2021-11-02 22:30:07+05:30 Cprogrammer Exp mbhangui $";
