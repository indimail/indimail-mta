...
static const char rcsid[]="$Id: searchinfo.c,v 1.13 2003/07/16 23:09:29 mrsam Exp $";
...
const char *keyword;
	...
	struct searchinfo *si;
		...
		if (t->tokentype != IT_ATOM &&
			...
			return (0);
		si->cs=(*unicode_ISO8859_1.toupper_func)
			(&unicode_ISO8859_1, t->tokenbuf, 0);
		if (!si->cs)
			write_error_exit(0);
		t=nexttoken_okbracket();
		if (t->tokentype != IT_ATOM &&
...
void search_set_charset_conv(struct searchinfo *si,
			     ...
			     const struct unicode_info *textcharset)
{
	if (textcharset->search_chset)
		textcharset=textcharset->search_chset;
	...
	for (; si; si=si->next)
	{
		...
		if (si->bs_charset &&
		    strcmp(si->bs_charset->chset, textcharset->chset) == 0)
			continue;	/* Already converted to this charset */

		si->bs_charset=0;
		if (si->bs)	free(si->bs);
		si->bs=0;

		p=unicode_convert(si->as ? si->as:"",
				       mycharset,
				       textcharset);
		...
		if (p)
		{
			si->bs=(*textcharset->toupper_func)(textcharset,
							    p, 0);
			free(p);
			if (!si->bs || maildir_search_start(&si->sei, si->bs))
				write_error_exit(0);
		}

		si->bs_charset=textcharset;
	}
}
